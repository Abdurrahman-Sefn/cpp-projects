/**
 * @file Assignment.hpp
 * @brief Defines the Assignment class for managing course assignments in the Educational Management
 * System.
 */
...
/**
 * @class Assignment
 * @brief Represents an assignment within a course, including its content, title, full mark, and
 * submission status.
 *
...
class Assignment {
    ...
    int fullMark;        /**< The maximum possible score for the assignment. */
    bool isAvailableToSubmit =
        true; /**< Flag indicating if the assignment is currently open for submissions. */
    const Course
        *const course; /**< A constant pointer to the parent Course this assignment belongs to. */
   public:
    /**
     * @brief Prints the content of the assignment.
     * @param tabs The number of tabs to indent the output.
     */
    void printContent(int tabs) const;
     ...
     */
    void listSolutions(const std::vector<AssignmentSolution *> &solutions, int tabs,
                       bool numbered) const;
     ...
     */
    Assignment(const Course *course, const std::string &title, const std::string &content,
               int fullMark);
     ...
     */
    const Course *getParentCourse() const;
};